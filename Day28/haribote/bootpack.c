...
void HariMain(void)
{
	...
	*((int *) 0x0fec) = (int) &fifo;
	...
	io_out8(PIC0_IMR, 0xf8); /*PIT和PIC1的键盘设置为许可（11111000） */
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	if (finfo != 0) {
		...
		for (i = 0; i < 16 * 256; i++) {
			nihongo[i] = hankaku[i]; /* 没有字体库，半角部分直接复制英文字库 */
		}
			...
			if (256 <= i && i <= 511) { /* 键盘数据 */
				...
				if (i == 256 + 0x3b && key_shift != 0 && key_win != 0) {
					...
					if (task != 0 && task->tss.ss0 != 0) {	/* Shift+F1 */
						...
						io_sti();
						task_run(task, -1, 0);	/* 为了确保执行结束处理，如果处于休眠状态则唤醒*/
					}
				}
								...
								if (0 <= x && x < sht->bxsize && 0 <= y && y < sht->bysize) {
									if (sht->buf[y * sht->bxsize + x] != sht->col_inv) {
										...
										if (sht->bxsize - 21 <= x && x < sht->bxsize - 5 && 5 <= y && y < 19) {
											/* 点击 “X”按钮 */
											if ((sht->flags & 0x10) != 0) {	/* 判断该窗口是否为应用程序窗口 */
												task = sht->task;
												cons_putstr0(task->cons, "\nBreak(mouse) :\n");
												io_cli();	/* 强制结束处理时禁止任务切换 */
												task->tss.eax = (int) &(task->tss.esp0);
